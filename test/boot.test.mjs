...
// Signing up is an address and a password. The email carries a code rather than a link:
// on iPhone a link opens in Safari, not in the app on the home screen.
await withPage({ session: null }, async page => {
  await settle(page);
  ...
  check('  confirmation is on, so there is no session and the screen asks for the code',
    await page.evaluate(() => S.auth) === 'verify'
    && /check your email/i.test(await page.locator('#app h2').innerText())
    && await page.locator('#app input[name=code]').count() === 1);
  check('    naming the address it went to', /new@example\.com/.test(await page.innerText('#app'))
    && await page.locator('#app input[name=email]').inputValue() === 'new@example.com');
  check('    with a keypad for the code, and the phone offered it from the email',
    await page.locator('#app input[name=code]').getAttribute('inputmode') === 'numeric'
    && await page.locator('#app input[name=code]').getAttribute('autocomplete') === 'one-time-code');
  await page.locator('#app button:has-text("Send a new code")').click();
  await page.waitForTimeout(300);
  const again = await page.evaluate(() => self.__resends);
  check('    and offering to send a new one', again.length === 1 && again[0].email === 'new@example.com' && again[0].type === 'signup',
    JSON.stringify(again));

  // A wrong code is refused and leaves them where they are.
  await page.evaluate(() => { self.__MODE.otpError = 'Token has expired or is invalid'; self.__alerts = []; window.alert = m => self.__alerts.push(m); });
  await page.locator('#app input[name=code]').fill('000000');
  await page.locator('#app form button.primary').click();
  await page.waitForTimeout(400);
  check('  a wrong code says so and stays on the screen', await page.evaluate(() => S.auth) === 'verify'
    && /wrong or has expired/i.test((await page.evaluate(() => self.__alerts)).join(' ')));

  await page.evaluate(() => { self.__MODE.otpError = null; self.__otps = []; });
  await page.locator('#app input[name=code]').fill('123456');
  await page.locator('#app form button.primary').click();
  await page.waitForTimeout(700);
  const otp = await page.evaluate(() => self.__otps);
  check('  the right code is checked against the address', otp.length === 1 && otp[0].email === 'new@example.com'
    && otp[0].token === '123456' && otp[0].type === 'signup', JSON.stringify(otp));
  check('    and it lets them in', await page.evaluate(() => !!me() && S.auth !== 'verify'));
});
...
// Logging in before the code was typed goes back to the code screen, not a dead end.
await withPage({ session: null, unconfirmed: true }, async page => {
  await settle(page);
  await page.evaluate(() => { S.auth = 'login'; render(); });
  await page.waitForTimeout(250);
  await page.locator('#app input[name=id]').fill('new@example.com');
  ...
  await page.locator('#app form button.primary').click();
  await page.waitForTimeout(500);
  check('an unconfirmed login is sent to type the code', await page.evaluate(() => S.auth) === 'verify'
    && await page.locator('#app input[name=email]').inputValue() === 'new@example.com');
});

// Forgetting the password sends a code, and says nothing about who has an account.
await withPage({ session: null }, async page => {
  await settle(page);
  ...
  check('  and offers no recovery code', !/recovery/i.test(await page.innerText('#app')));
  await page.locator('#app input[name=email]').fill('ari@example.com');
  await page.locator('#app form button.primary').click();
  await page.waitForTimeout(400);
  const sent = await page.evaluate(() => self.__resets);
  check('  the code is asked for', sent.length === 1 && sent[0].email === 'ari@example.com', JSON.stringify(sent));
  check('  and the next screen takes it with the new password', await page.evaluate(() => S.auth) === 'resetcode'
    && await page.locator('#app input[name=code]').count() === 1
    && /if ari@example\.com has an account/i.test(await page.innerText('#app')));
  await page.locator('#app input[name=code]').fill('654321');
  await page.locator('#app input[name=password]').fill('brandnew1');
  await page.locator('#app input[name=again]').fill('brandnew1');
  await page.locator('#app form button.primary').click();
  await page.waitForTimeout(700);
  const otp = await page.evaluate(() => self.__otps);
  check('  the code is checked as a reset', otp.length === 1 && otp[0].type === 'recovery'
    && otp[0].email === 'ari@example.com' && otp[0].token === '654321', JSON.stringify(otp));
  const edits = await page.evaluate(() => self.__userEdits);
  check('  and the new password is set', edits.length === 1 && edits[0].password === 'brandnew1',
    JSON.stringify(edits));
  check('    which leaves them signed in', await page.evaluate(() => !!me()));
});
...
await withPage({ session: { user: { id: 'u1' } }, email: 'ari@users.quota.local' }, async page => {
  await settle(page);
  check('an account with a made-up address is asked for a real one',
    await page.evaluate(() => S.needEmail) === true && /add your email/i.test(await page.locator('#app h2').innerText())
    && await page.locator('#bar').isHidden() && await page.locator('.post').count() === 0);
  check('  saying why it matters', /forget your password/i.test(await page.innerText('#app')));

  // "Not now" lets them in, with a card that brings the screen back.
  await page.locator('#app button:has-text("Not now")').click();
  await page.waitForTimeout(400);
  check('  "Not now" lets them in', await page.locator('.post').count() > 0 && await page.locator('#bar').isVisible());
  check('    with a card on the feed that keeps asking', await page.locator('.warn-card').count() === 1);
  await page.locator('.warn-card button').click();
  await page.waitForTimeout(400);
  check('    which brings the screen back', /add your email/i.test(await page.locator('#app h2').innerText()));

  await page.locator('#app input[name=email]').fill('ari@example.com');
  await page.locator('#app form button.primary').click();
  await page.waitForTimeout(500);
  const edits = await page.evaluate(() => self.__userEdits);
  check('  adding one asks for a code', edits.length === 1 && edits[0].email === 'ari@example.com',
    JSON.stringify(edits));
  check('    and the screen takes it', await page.locator('#app input[name=code]').count() === 1
    && /ari@example\.com/.test(await page.innerText('#app')));
  await page.locator('#app input[name=code]').fill('111222');
  await page.locator('#app form button.primary').click();
  await page.waitForTimeout(500);
  const otp = await page.evaluate(() => self.__otps);
  check('  the code is checked as a change of address', otp.length === 1 && otp[0].type === 'email_change'
    && otp[0].email === 'ari@example.com' && otp[0].token === '111222', JSON.stringify(otp));
  check('    and then it stops asking', await page.evaluate(() => S.needEmail) === false
    && await page.locator('.warn-card').count() === 0 && await page.locator('.post').count() > 0);
});
await withPage({ session: { user: { id: 'u1' } }, email: 'ari@example.com' }, async page => {
  ...
  check('an account that already has one is not asked', await page.evaluate(() => S.needEmail) === false
    && await page.locator('.warn-card').count() === 0 && await page.locator('.post').count() > 0);
  // And it can change it from Settings, through the same screen.
  await page.evaluate(() => { S.tab = 'profile'; S.settings = true; render(); });
  await page.waitForTimeout(300);
  await page.locator('.li:has-text("Email")').click();
  await page.waitForTimeout(300);
  check('  Settings changes it through the same screen', /change your email/i.test(await page.locator('#app h2').innerText())
    && /ari@example\.com/.test(await page.innerText('#app')));
  await page.locator('#app button:has-text("Cancel")').click();
  await page.waitForTimeout(300);
  check('    and Cancel goes back', await page.evaluate(() => S.editEmail) === false && await page.evaluate(() => S.settings) === true);
});
